-- SAFE TO RUN ANYTIME - This SQL is idempotent (can run multiple times safely)
-- It uses "ON CONFLICT DO NOTHING" to prevent duplicates

-- Insert all 10 achievements (will skip if already exists)
insert into public.achievements (id, name, description, icon, tier) values
  ...
  -- New 5
  ('hard_mode_master', 'Hard Mode Master', 'Complete a quiz on hard difficulty', '💪', 'silver'),
  ('quiz_marathon', 'Quiz Marathon', 'Complete 5 quizzes in one day', '🏃', 'silver'),
  ('speed_demon', 'Speed Demon', 'Complete a 10-question quiz in under 2 minutes', '⚡', 'gold'),
  ('knowledge_seeker', 'Knowledge Seeker', 'Complete quizzes in 10 different categories', '📚', 'gold'),
  ('perfectionist', 'Perfectionist', 'Achieve 3 perfect scores', '✨', 'platinum')
on conflict (id) do nothing;

-- Update the achievement checking function (safe to run multiple times)
...
begin
  ...
  -- New: Check unique categories
  select count(distinct category) into v_categories_count
  from public.quiz_sessions where echo_user_id = p_echo_user_id;

  -- New: Check quizzes completed today
